...
class PostClosingJournalUseCase:
    """
    Use case untuk posting jurnal penutup.
    """
    ...
    async def execute(self, command: PostClosingJournalCommand) -> CommandResult:
        ...
        try:
            period_status = await self._period_service.get_period_status(
                command.legal_entity_id, period_str
            )
            if period_status not in (PeriodStatus.OPEN.value, PeriodStatus.PRE_CLOSING.value):
                raise ValueError(f"Period {period_str} cannot be closed (status={period_status})")
            ...
            for acc in revenue_accounts:
                revenue_balance += await self._ledger_repo.get_account_balance(
                    ...
                    as_of_date=command.closing_date,
                )
            ...
            async def _execute():
                result = await self._journal_service.post_closing_journal(
                    request=request,
