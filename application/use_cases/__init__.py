...
from application.use_cases.aml_screening_transaction import (
    AMLScreeningCommand, AMLScreeningUseCase, aml_screening_handler
)
from application.use_cases.ap_payment_run import (
    APPaymentRunCommand, APPaymentRunUseCase, ap_payment_run_handler
)
from application.use_cases.approve_journal_four_eyes import (
    ApproveJournalCommand, ApproveJournalUseCase, approve_journal_handler
)
from application.use_cases.ar_collection_workflow import (
    ARCollectionWorkflowCommand, ARCollectionWorkflowUseCase, ar_collection_workflow_handler
)
from application.use_cases.bank_reconciliation import (
    BankReconciliationCommand, BankReconciliationUseCase, bank_reconciliation_handler
)
from application.use_cases.budget_vs_actual_analysis import (
    BudgetVsActualCommand, BudgetVsActualUseCase, budget_vs_actual_handler
)
from application.use_cases.cogs_calculation import (
    COGSCalculationCommand, COGSCalculationUseCase, cogs_calculation_handler
)
from application.use_cases.consolidation_group_report import (
    ConsolidationGroupReportCommand, ConsolidationGroupReportUseCase, consolidation_group_report_handler
)
from application.use_cases.coretax_bulk_submission import (
    CoretaxBulkSubmissionCommand, CoretaxBulkSubmissionUseCase, coretax_bulk_submission_handler
)
from application.use_cases.depreciation_monthly_run import (
    DepreciationMonthlyRunCommand, DepreciationMonthlyRunUseCase, depreciation_monthly_run_handler
)
from application.use_cases.disaster_recovery_replay import (
    DisasterRecoveryReplayCommand, DisasterRecoveryReplayUseCase, disaster_recovery_replay_handler
)
from application.use_cases.financial_statement_generation import (
    FinancialStatementGenerationCommand, FinancialStatementGenerationUseCase, financial_statement_generation_handler
)
from application.use_cases.fiscal_reconciliation import (
    FiscalReconciliationCommand, FiscalReconciliationUseCase, fiscal_reconciliation_handler
)
from application.use_cases.forex_revaluation import (
    ForexRevaluationCommand, ForexRevaluationUseCase, forex_revaluation_handler
)
from application.use_cases.hedge_accounting_execution import (
    HedgeAccountingCommand, HedgeAccountingUseCase, hedge_accounting_handler
)
from application.use_cases.hpp_manufacturing_close import (
    HPPManufacturingCloseCommand, HPPManufacturingCloseUseCase, hpp_manufacturing_close_handler
)
from application.use_cases.impairment_testing_annual import (
    ImpairmentTestingCommand, ImpairmentTestingUseCase, impairment_testing_handler
)
from application.use_cases.intercompany_elimination import (
    IntercompanyEliminationCommand, IntercompanyEliminationUseCase, intercompany_elimination_handler
)
from application.use_cases.payroll_monthly_run import (
    PayrollMonthlyRunCommand, PayrollMonthlyRunUseCase, payroll_monthly_run_handler
)
from application.use_cases.period_close import (
    PeriodCloseCommand, PeriodCloseUseCase, period_close_handler
)
from application.use_cases.period_reopen_with_audit import (
    PeriodReopenWithAuditCommand, PeriodReopenWithAuditUseCase, period_reopen_handler
)
from application.use_cases.post_adjusting_journal import (
    PostAdjustingJournalCommand, PostAdjustingJournalUseCase, post_adjusting_journal_handler
)
from application.use_cases.post_closing_journal import (
    PostClosingJournalCommand, PostClosingJournalUseCase, post_closing_journal_handler
)
from application.use_cases.post_journal_entry import (
    PostJournalEntryCommand, PostJournalEntryUseCase, post_journal_entry_handler
)
from application.use_cases.reverse_journal import (
    ReverseJournalCommand, ReverseJournalUseCase, reverse_journal_handler
)
from application.use_cases.stock_opname_cycle import (
    StockOpnameCycleCommand, StockOpnameCycleUseCase, stock_opname_cycle_handler
)
from application.use_cases.tax_filing_submission import (
    TaxFilingSubmissionCommand, TaxFilingSubmissionUseCase, tax_filing_submission_handler
)
from application.use_cases.year_end_closing import (
    YearEndClosingCommand, YearEndClosingUseCase, year_end_closing_handler
)
...
def _make_wrapper(handler, use_case_cls):
    """Buat wrapper yang mengambil use case dari container global."""
