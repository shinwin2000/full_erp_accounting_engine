...
class LedgerService:
    ...
    async def post_journal(
        ...
        journal_entry = JournalEntry(
            journal_id=uuid4(),
            journal_number="",  # generated by repo
        ...
        if self._event_publisher:
            try:
                ...
                logger.debug(f"Published JournalPostedEvent for {journal_id}")
    ...
    async def close_period(
        ...
        # Publish PeriodClosedEvent
        if self._event_publisher:
            try:
                # Parse period: YYYY-MM -> year, month
                year, month = map(int, period.split("-"))
                event = PeriodClosedEvent(
                    period_id=uuid4(),  # This should be the actual period ID from repo
                    legal_entity_id=legal_entity_id,
                    period_year=year,
                    period_month=month,
                    user_id=str(user_id),
                ...
                logger.debug(f"Published PeriodClosedEvent for {period}")
            except Exception as e:
    ...
    async def reopen_period(
        ...
        if self._event_publisher:
            try:
                ...
                await self._event_publisher.publish(event, correlation_id)
                logger.debug(f"Published PeriodReopenedEvent for {period}")
            except Exception as e:
