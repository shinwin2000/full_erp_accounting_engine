#!/usr/bin/env python3
"""
checker/ethics_checker.py
=========================
Static checker untuk modul-modul di compliance/ethics/.
...
Usage:
    python -m checker.ethics_checker --verbose
    python -m checker.ethics_checker --json report.json
"""
...
# Disesuaikan dengan definisi aktual yang ada di modul (berdasarkan output checker sebelumnya)
MODULE_EXPECTATIONS = {
...
class EthicsChecker:
    def __init__(self, ethics_path: Path | None = None):
        if ethics_path is None:
            ethics_path = ROOT / "compliance" / "ethics"
        self.ethics_path = Path(ethics_path)
    ...
    def check_all(self) -> dict[str, Any]:
        if not self.ethics_path.exists():
            self.errors.append({
                "file": str(self.ethics_path),
                "error": "Folder compliance/ethics/ tidak ditemukan"
            })
            return self._result()

        py_files = {}
        for py_file in self.ethics_path.glob("*.py"):
    ...
    def _check_init(self):
        init_file = self.ethics_path / "__init__.py"
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.ImportFrom):
                # Deteksi import relatif maupun absolut
                if node.level > 0:
                    # Relative import: from . import xxx atau dari ..xxx import yyy
                    ...
                    # Absolute import: from compliance.ethics.xxx import ...
                    module_parts = node.module.split('.')
                    if module_parts[0] == 'compliance' and len(module_parts) >= 3 and module_parts[1] == 'ethics':
        ...
        # Tidak memeriksa __all__ agar tidak menambah warning
...
def check_ethics(ethics_path: Path | None = None) -> dict[str, Any]:
    checker = EthicsChecker(ethics_path)
...
def integrate_with_rca(engine=None):
    ...
    class StaticEthicsRule(RCARule):
        def __init__(self):
            super().__init__(priority=195, category=Category.DDD, name="StaticEthicsRule")
            self._checker = EthicsChecker()

        def match(self, exc, frames, context) -> bool:
            return "Ethics" in type(exc).__name__ or "ethics" in str(exc).lower()
        ...
        def analyze(self, exc, frames, context) -> RCAResult | None:
            ...
            if result["errors"]:
                ...
                return RCAResult(
                    ...
                    root_cause="Pelanggaran etika terdeteksi secara statis: " + "; ".join(error_msgs[:3]),
                    evidence=error_msgs,
                    impact=["Kepatuhan etika sistem tidak terpenuhi."],
                    suggested_fix="Periksa compliance/ethics/ dan pastikan semua komponen terdefinisi dengan benar.",
                    raw_error=str(exc),
                    confidence=0.90
                )
            return None
    ...
    if engine is None:
        engine = get_engine()
    engine.register_rule(StaticEthicsRule())
    return engine
...
def print_report(result: dict[str, Any], verbose: bool = False):
    ...
    print(f"\n{c['BOLD']}{c['CYAN']}╔{'═'*72}╗")
    print("║         ETHICS STATIC CHECKER — v2.3                  ║")
    print(f"╚{'═'*72}╝{c['RESET']}")

    print(f"\n  📁 Ethics Path: {ROOT / 'compliance' / 'ethics'}")
...
def main():
    parser = argparse.ArgumentParser(description="Ethics Static Checker")
