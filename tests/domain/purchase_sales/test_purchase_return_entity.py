# tests/domain/purchase_sales/test_purchase_return_entity.py
"""
Comprehensive unit tests for Purchase Return Entity.

Covers:
- PurchaseReturnItem value object (construction, validation, properties, serialization)
- PurchaseReturnEntity (construction, validation, item management, status transitions, serialization)
...
def valid_item_kwargs() -> dict[str, Any]:
    """Valid arguments for creating a PurchaseReturnItem."""
    return {
        "item_id": uuid4(),
        "item_code": "ITEM-001",
...
@pytest.fixture
def valid_item(valid_item_kwargs) -> PurchaseReturnItem:
    return PurchaseReturnItem(**valid_item_kwargs)


@pytest.fixture
def valid_return_kwargs(valid_item) -> dict[str, Any]:
    """Valid arguments for creating a PurchaseReturnEntity."""
    now = datetime.now(UTC)
    total = valid_item.total_amount  # 5 * 100 = 500
    return {
        "return_id": uuid4(),
        "return_number": "PR-2026-001",
        "invoice_id": uuid4(),
...
@pytest.fixture
def valid_return(valid_return_kwargs) -> PurchaseReturnEntity:
    return PurchaseReturnEntity(**valid_return_kwargs)


@pytest.fixture
def another_item() -> PurchaseReturnItem:
...
# -----------------------------------------------------------------------------
# Tests for PurchaseReturnItem (Value Object)
# -----------------------------------------------------------------------------

class TestPurchaseReturnItem:
    """Test the PurchaseReturnItem immutable value object."""

    def test_construction_success(self, valid_item_kwargs):
        item = PurchaseReturnItem(**valid_item_kwargs)
        assert isinstance(item, PurchaseReturnItem)
...
# -----------------------------------------------------------------------------
# Tests for PurchaseReturnEntity
# -----------------------------------------------------------------------------

class TestPurchaseReturnEntity:
    """Test the PurchaseReturnEntity aggregate."""

    def test_construction_success(self, valid_return_kwargs):
        entity = PurchaseReturnEntity(**valid_return_kwargs)
        assert entity.return_id == valid_return_kwargs["return_id"]
        assert entity.status == PurchaseReturnStatus.DRAFT
    ...
    def test_approve(self, valid_return):
        ...
        assert entity2.status == PurchaseReturnStatus.APPROVED
    ...
    def test_complete(self, valid_return):
        ...
        assert entity2.status == PurchaseReturnStatus.COMPLETED
    ...
    def test_cancel(self, valid_return):
        ...
        assert entity2.status == PurchaseReturnStatus.CANCELLED
...
class TestPurchaseReturnRepository:
    ...
    def test_methods_raise_not_implemented(self):
        ...
        with pytest.raises(NotImplementedError):
            repo.get_by_number("PR-123", uuid4())
        with pytest.raises(NotImplementedError):
