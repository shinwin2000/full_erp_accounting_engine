...
- All async repository methods tested with proper mocking.
"""
...
def mock_datetime_now():
    """Mock datetime.now() and UTC for all tests."""
...
def create_credit_limit(amount: Decimal = Decimal("10000000")) -> CustomerCreditLimitVO:
...
def create_tax_status(npwp: str = "1234567890") -> CustomerTaxStatusVO:
...
class TestExceptions:
    def test_customer_aggregate_error(self):
        with pytest.raises(CustomerAggregateError, match="test error"):
            raise CustomerAggregateError("test error")

    def test_duplicate_customer_code_error(self):
        with pytest.raises(DuplicateCustomerCodeError, match="duplicate code"):
            raise DuplicateCustomerCodeError("duplicate code")
    ...
    def test_customer_not_found_error(self):
        with pytest.raises(CustomerNotFoundError, match="not found"):
            raise CustomerNotFoundError("not found")

    def test_invalid_customer_status_transition_error(self):
        with pytest.raises(InvalidCustomerStatusTransitionError, match="invalid transition"):
            raise InvalidCustomerStatusTransitionError("invalid transition")
...
class TestCustomerAggregate:
    ...
    def test_get_customer_by_id_not_found(self):
        agg = create_test_aggregate()
        assert agg.get_customer(uuid.uuid4()) is None
    ...
    def test_get_customer_by_code_not_found(self):
        agg = create_test_aggregate()
        assert agg.get_customer_by_code("MISSING") is None
    ...
    def test_get_customer_by_email_not_found(self):
        agg = create_test_aggregate()
        assert agg.get_customer_by_email("missing@example.com") is None
    ...
    def test_get_customer_by_tax_id_not_found(self):
        agg = create_test_aggregate()
        assert agg.get_customer_by_tax_id("9999999999") is None
    ...
    def test_get_customers_by_segment(self):
        ...
        assert len(by_seg) == 1
        assert by_seg[0].segment == CustomerSegment.RETAIL
    ...
    def test_get_customers_on_credit_hold(self):
        ...
        assert on_hold[0].customer_code == "CUST-001"

    @pytest.mark.parametrize("threshold,expected_count", [(70, 1), (50, 2), (90, 0)])
    def test_get_high_risk_customers(self, threshold, expected_count):
        agg = create_test_aggregate()
        c1 = create_test_customer(risk_score=80)
        c2 = create_test_customer(customer_code="CUST-002", risk_score=50)
        agg = agg.add_customer(c1, "tester")
        agg = agg.add_customer(c2, "tester")
        high_risk = agg.get_high_risk_customers(threshold=threshold)
    ...
    def test_get_total_outstanding_balance_empty(self):
        agg = create_test_aggregate()
        assert agg.get_total_outstanding_balance() == Decimal("0")
    ...
    def test_update_customer_status_transition(self):
        ...
        assert new_agg.version == agg.version + 1
        # Should have at least 2 events: created + status changed
    ...
    def test_record_customer_payment_negative_amount_raises(self):
        agg = create_test_aggregate()
    ...
    def test_post_unknown_type_raises(self):
        ...
        with pytest.raises(ValueError, match="Unknown transaction type"):
            agg.post(c1.customer_id, Decimal("1000"), "tester", "unknown")
    ...
    def test_can_approve(self, role, expected):
        ...
        c1 = create_test_customer(status=CustomerStatus.DRAFT)
        agg = agg.add_customer(c1, "tester")
        assert agg.can_approve(c1.customer_id, role) == expected

    def test_can_approve_active_customer_returns_false(self):
        agg = create_test_aggregate()
        c1 = create_test_customer(status=CustomerStatus.ACTIVE)
        agg = agg.add_customer(c1, "tester")
        assert agg.can_approve(c1.customer_id, "finance_manager") is False
    ...
    def test_approve_fails_if_not_authorized(self):
        ...
        with pytest.raises(CustomerAggregateError, match="Cannot approve"):
            agg.approve(c1.customer_id, "user")
    ...
    def test_can_reject(self):
        ...
        assert agg.can_reject(c1.customer_id, "user") is False
    ...
    def test_can_reverse_always_false(self):
        ...
        c1 = create_test_customer()
        agg = agg.add_customer(c1, "tester")
        assert agg.can_reverse(c1.customer_id) is False
    ...
    def test_reverse_raises_not_implemented(self):
        ...
        with pytest.raises(NotImplementedError):
            agg.reverse(c1.customer_id, "user", "reason")

    @pytest.mark.parametrize("status,expected", [
        (CustomerStatus.ACTIVE, True),
        (CustomerStatus.INACTIVE, False),
    ])
    def test_can_close(self, status, expected):
        agg = create_test_aggregate()
        c1 = create_test_customer(status=status)
        agg = agg.add_customer(c1, "tester")
        assert agg.can_close(c1.customer_id) == expected
    ...
    def test_can_reopen(self, status, expected):
        agg = create_test_aggregate()
        c1 = create_test_customer(status=status)
        agg = agg.add_customer(c1, "tester")
        assert agg.can_reopen(c1.customer_id) == expected
    ...
    def test_update_method(self):
        agg = create_test_aggregate()
        new_legal_id = uuid.uuid4()
        new_agg = agg.update("updater", legal_entity_id=new_legal_id)
        assert new_agg.legal_entity_id == new_legal_id
        assert new_agg.version == agg.version + 1
...
class TestCustomerAggregateRepository:
    ...
    async def test_update(self, agg):
        ...
        retrieved = await CustomerAggregateRepository.get_by_id(agg.aggregate_id)
        assert retrieved.legal_entity_id == new_legal_id
