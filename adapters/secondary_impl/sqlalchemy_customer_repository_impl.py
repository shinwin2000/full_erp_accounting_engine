...
class CustomerTable(Base):
    ...
    status = Column(String(20), nullable=False, default="ACTIVE")  # ACTIVE, INACTIVE, BLACKLISTED
...
class SQLAlchemyCustomerRepository(CustomerRepositoryPort):
    ...
    # ==================== DELETE METHOD (sesuai port) ====================

    async def delete(self, customer_id: UUID, user_id: UUID | None = None, permanent: bool = False) -> bool:
        """
        Delete a customer.
        - If permanent=True: hard delete from database.
        - If permanent=False: soft delete (set deleted_at and is_active=False).
        """
        session = await self._get_session()
        async with session.begin():
            if permanent:
    ...
    async def get_statistics(self, legal_entity_id: UUID) -> dict[str, Any]:
        ...
        blacklisted_stmt = select(func.count()).where(
            CustomerTable.legal_entity_id == legal_entity_id,
            CustomerTable.status == "BLACKLISTED",
            CustomerTable.deleted_at.is_(None),
        )
        blacklisted = await session.scalar(blacklisted_stmt) or 0
    ...
    async def blacklist(self, customer_id: UUID, reason: str, blacklisted_by: str) -> bool:
        ...
        async with session.begin():
            ...
            row.blacklisted_at = datetime.utcnow()
            row.updated_at = datetime.utcnow()
            await session.flush()
