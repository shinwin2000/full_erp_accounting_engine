...
class ConsolidationGroupReportAdapter(ConsolidationGroupReportPort):
    ...
    async def get_nci_breakdown(self, group_id: UUID) -> dict[str, Any]:
        ...
        # Asumsikan service memiliki method get_nci_breakdown
    ...
    # Method tambahan jika diperlukan
    async def generate_report(self, group_id: UUID, period_start: date, period_end: date,
                              include_intercompany: bool = True, include_nci: bool = True) -> dict[str, Any]:
        service = await self._get_service()
        # Asumsikan service memiliki get_consolidated_report
