...
async def get_ap_svc(
    ...
    try:
        ...
        svc = await container.resolve_async(APService)
        svc._ap_repo.session = session
        return svc
    except ImportError as e:
...
async def get_ap_payment_run_use_case(
    request: Request,
    session: AsyncSession = Depends(get_async_session),
) -> Any:
    ...
    try:
        ...
        container = request.app.state.container
        svc = await container.resolve_async(APPaymentRunUseCase)
        if hasattr(svc, '_ap_repo'):
            svc._ap_repo.session = session
        return svc
    except ImportError as e:
