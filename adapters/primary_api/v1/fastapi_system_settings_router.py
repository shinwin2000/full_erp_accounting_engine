...
# FIX: these exception classes are raised by SystemSettingsService for
# perfectly normal, expected conditions (setting is read-only, setting is
# locked, setting not found, value fails validation) but no handler in this
# router ever caught them by name - only generic ValueError/PermissionError/
# Exception. That meant every one of these expected conditions surfaced to
# the client as "500 Internal Server Error" instead of a meaningful 4xx
# status (e.g. updating company.name, which is read-only by design, threw a
# 500 instead of a 409/403). Imported here (module level, not the lazy
# per-request import used for the service class itself) so every handler
# below can catch them specifically.
...
async def get_settings_svc(request: Request) -> Any:
    ...
    return await container.resolve_async(SystemSettingsService)
...
async def create_setting(
    ...
    except SystemSettingsError as e:

        # FIX: raised e.g. when the key already exists - was previously
...
async def update_setting(
    ...
    except SettingReadonlyError as e:

        raise HTTPException(status_code=409, detail=str(e))

    except SettingLockedError as e:
