...
async def get_ar_svc(
    ...
    svc = await container.resolve_async(ARService)
    svc._ar_repo.session = session
    return svc


async def get_ar_collection_workflow(
    request: Request,
    session: AsyncSession = Depends(get_async_session),
) -> Any:
    ...
    container = request.app.state.container
    svc = await container.resolve_async(ARCollectionWorkflowUseCase)
    # Asumsi use case ini memiliki repo yang juga butuh session, set jika ada
    if hasattr(svc, '_ar_repo'):
        svc._ar_repo.session = session
    return svc
...
async def create_ar_invoice(
    ...
    try:
        ...
        response = ARInvoiceResponseSchema(
            id=result.id,
...
async def delete_ar_invoice(
    ...
    try:
        ...
        response = ARInvoiceActionResponseSchema(
...
async def create_ar_credit_note(
    ...
    if idempotency_key:
        ...
        if cached is not None:
            logger.info(f"Idempotent cache hit: {method_name} key={idempotency_key[:8]}...")
            return ARCreditNoteResponseSchema(**cached)
