...
async def get_db_session():
    ...
    CATATAN: router ini SEBELUMNYA tidak punya dependency session sama
    sekali - ConsolidationService/self._le_repo mengandalkan session yang
    di-set dari luar (pola yang sama seperti fastapi_iam_router.py), tapi
    tidak ada satupun endpoint yang menyediakannya -> "Session not set"
    di setiap operasi yang menyentuh DB lewat repo.
    """
...
async def get_consolidation_service(request: Request) -> Any:
    ...
    return await container.resolve_async(ConsolidationService)
