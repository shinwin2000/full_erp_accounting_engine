...
class CostCardTable(Base):
    __tablename__ = "cost_card_work_order"
    __table_args__ = (
        Index("idx_cost_card_wo_work_order", "work_order_id"),
        Index("idx_cost_card_wo_product", "product_id"),
    )
...
class CostCardPerWorkOrder:
    ...
    async def compute_cost_card(self, work_order_id: UUID, legal_entity_id: UUID) -> dict[str, Any]:
        async with await self._get_session() as session:
            ...
            # Query biaya dengan fallback 0 jika tabel belum ada
            try:
                material_stmt = text("""
                    SELECT COALESCE(SUM(quantity * unit_cost), 0) as total_material
                    FROM material_issue WHERE work_order_id = :wo_id AND deleted_at IS NULL
                """)
            ...
            try:
                labor_stmt = text("""
                    ...
                    FROM labor_record WHERE work_order_id = :wo_id AND deleted_at IS NULL
                """)
            ...
            try:
                machine_stmt = text("""
                    ...
                    FROM machine_usage WHERE work_order_id = :wo_id AND deleted_at IS NULL
                """)
            ...
            try:
                overhead_stmt = text("""
                    ...
                    FROM overhead_allocation WHERE work_order_id = :wo_id AND deleted_at IS NULL
                """)
    ...
    async def get_variance_analysis(
        ...
        return {
            ...
            "variance_percent": (total_variance / total_standard * 100) if total_standard != 0 else 0,
