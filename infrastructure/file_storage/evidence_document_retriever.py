...
Responsibility: Layanan untuk mengambil (retrieve) dokumen bukti dari file storage
               dengan caching dan presigned URL generation. Mendukung retrieval
               dari hot storage (MinIO/S3) dan cold storage (Glacier).
               Juga menyediakan fungsi untuk batch retrieval dan streaming.
Dependencies:
- asyncio, logging, tempfile
...
Audit: Setiap retrieval evidence dicatat untuk audit trail.
       Akses ke dokumen sensitif dicatat secara khusus.
"""
...
# Cache TTL untuk presigned URLs (seconds)
PRESIGNED_URL_CACHE_TTL = 300  # 5 minutes

# Batch retrieval limits
MAX_BATCH_SIZE = 50
...
class EvidenceDocumentRetriever:
    """
    Layanan retrieval dokumen bukti.

    Fitur:
    - Retrieve evidence dari hot storage (MinIO/S3)
    - Retrieve dari cold storage (Glacier) dengan job tracking
    - Generate presigned URLs untuk akses sementara
    - Batch retrieval untuk multiple files
    - Caching untuk presigned URLs
    ...
    async def retrieve_evidence(
        ...
        """
        Retrieve evidence document content.

        Args:
            evidence_uri: URI of the evidence
            verify_hash: Verify file integrity
            use_cache: Use cached result (for repeated accesses)

        Returns:
            File content as bytes
        """
        # Check if already in cache (for large files, we might not cache)
    ...
    async def _retrieve_from_hot(self, evidence_uri: str, verify_hash: bool) -> bytes:
        """Retrieve from hot storage (MinIO/S3)."""
    ...
    async def _retrieve_from_cold(self, evidence_uri: str, verify_hash: bool) -> bytes:
        """Retrieve from cold storage (Glacier)."""
        storage = await self._get_cold_storage()

        # Check if already in retrieval job
        ...
        if job_status and job_status.get("status") == "Succeeded":
            ...
            # Initiate new retrieval
            # Glacier retrieval is async, so we initiate and raise
            await storage.download(evidence_uri)  # This will initiate retrieval
            raise EvidenceRetrievalError(
    ...
    async def get_presigned_url(
        ...
        Args:
            ...
            force_refresh: Generate new URL even if cached
        ...
        try:
            if self._is_hot_uri(evidence_uri):
                ...
                # Glacier doesn't support presigned URLs directly
    ...
    async def batch_retrieve(
        ...
        """
        Retrieve multiple evidence documents in batch.

        Args:
            evidence_uris: List of evidence URIs
            verify_hash: Verify file integrity

        Returns:
            Dict mapping URI to content
        """
    ...
    async def get_evidence_metadata(self, evidence_uri: str) -> dict[str, Any]:
        """
        Get metadata for evidence document.
        """
        try:
            if self._is_hot_uri(evidence_uri):
                storage = await self._get_hot_storage()
    ...
    async def check_availability(self, evidence_uri: str) -> dict[str, Any]:
        """
        Check if evidence is available and estimate retrieval time if in cold storage.
        """
        try:
            if self._is_hot_uri(evidence_uri):
                storage = await self._get_hot_storage()
                ...
                if job_status and job_status.get("status") == "Succeeded":
                    ...
                    return {
                        ...
                        "estimated_retrieval_seconds": 360,  # Standard retrieval time
                    }
            else:
...
async def get_evidence_retriever() -> EvidenceDocumentRetriever:
    """Get singleton instance of EvidenceDocumentRetriever."""
...
async def get_evidence_retriever_dep():
    """FastAPI dependency for evidence retriever."""
