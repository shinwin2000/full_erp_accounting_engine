#!/usr/bin/env python3
"""
Module: journal_line_partitioned_table.py
Layer: Infrastructure (Persistence ORM)
Responsibility: ORM table definition for 'journal_line_partitioned'.
...
class JournalLinePartitionedTable(Base):
    """ORM model for table: journal_line_partitioned."""

    __tablename__ = "journal_line_partitioned"

    id: Mapped[UUID] = mapped_column(PG_UUID(as_uuid=True), primary_key=True, server_default=text("gen_random_uuid()"))
    journal_header_id: Mapped[UUID] = mapped_column(PG_UUID(as_uuid=True), nullable=False)
    ...
    exchange_rate: Mapped[Decimal | None] = mapped_column(Numeric(20, 6), nullable=True)
    ...
    def __repr__(self) -> str:
        pk = getattr(self, "id", "?")
        return f"<JournalLinePartitionedTable id={pk}>"
