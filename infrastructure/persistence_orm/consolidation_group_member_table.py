...
class ConsolidationGroupMemberTable(Base, TimestampMixin, SoftDeleteMixin):
    ...
    group_id: Mapped[uuid.UUID] = mapped_column(PGUUID(as_uuid=True), ForeignKey("consolidation_group.id", ondelete="CASCADE"), nullable=False, index=True)
    entity_id: Mapped[uuid.UUID] = mapped_column(PGUUID(as_uuid=True), ForeignKey("legal_entity.id", ondelete="CASCADE"), nullable=False, index=True)
    ...
    group: Mapped[ConsolidationGroupTable] = relationship(
        "ConsolidationGroupTable", back_populates="members"
    )
    entity: Mapped[LegalEntityTable] = relationship(
        "LegalEntityTable", back_populates="consolidation_members"
    )
