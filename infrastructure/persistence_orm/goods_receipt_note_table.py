...
class GoodsReceiptNoteTable(Base, TimestampMixin, SoftDeleteMixin, VersionMixin, LegalEntityMixin):
    ...
    purchase_order_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("purchase_order.id", use_alter=True, name="fk_grn_purchase_order"),
        nullable=False
    )
    receipt_date: Mapped[date] = mapped_column(Date, nullable=False)
    supplier_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("supplier.id", use_alter=True, name="fk_grn_supplier"),
        nullable=False
    )
    ...
    supplier: Mapped[SupplierTable] = relationship("SupplierTable", back_populates="goods_receipt_notes")
    purchase_order: Mapped[PurchaseOrderTable] = relationship("PurchaseOrderTable", back_populates="goods_receipt_notes")
    ...
    def cancel(self, record_event: bool = True) -> None:
        ...
        if self.status == "confirmed":
            # boleh cancel setelah confirmed, tapi harus ada otorisasi (di sini langsung diizinkan)
            pass
...
class GoodsReceiptNoteLineTable(Base):
    ...
    grn_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("goods_receipt_note.id", ondelete="CASCADE", use_alter=True, name="fk_grn_line_grn"),
        nullable=False
    )
    ...
    grn: Mapped[GoodsReceiptNoteTable] = relationship("GoodsReceiptNoteTable", back_populates="lines")
